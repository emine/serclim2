...
// update signature does not work			
					...
					tx.executeSql(sql, [id_intervention], function (tx, results) {
						...
						for (var i = 0; i < nquest; i++) {
							(function(i) {
								...
								var sql = 	'SELECT q.id_question, q.id_questionnaire, q.type, q.question, q.options, q.reponse_standard, r.reponse ' + 
...
//			$("#grid-devis").append(
//				'<div  style="width:60%" class="stock-item' + obj.id_stock + ' ui-block-a"><div class="ui-bar ui-bar-a">' + obj.designation + '</div></div>' +
//				'<div style="width:10%" class="stock-item' + obj.id_stock + ' ui-block-b"><div class="ui-bar ui-bar-a">' + obj.unite + '</div></div>' +
//				'<div style="width:25%;text-align:right" class="stock-item' + obj.id_stock + ' ui-block-c"><div class="ui-bar ui-bar-a">' + prix_article(obj) + '</div></div>' +
//				'<div style="width:5%" class="stock-item' + obj.id_stock + ' ui-block-c"><div class="ui-bar ui-bar-a"><span stock-item="' + obj.id_stock + '" class="ui-icon-delete ui-btn-icon-right"><span></div></div>') ;
		}			
	}
...
//	canvas.onmouseup = function() {
	...
	prix_article = function(obj) {
		...
		// devis
		return	parseFloat((obj.prix + obj.heures * ej.data.intervention.taux_horaire) * 
...
//		}	
		...
		ej.db.transaction(function (tx) {
			var sql = 'UPDATE interventions SET date_heure_fin = ?, statut = ?, signature = ?, observation = ? WHERE id_intervention = ?' ;
//			var replace = [datetime, statut, canvasData, observation, ej.data.intervention.id_intervention] ;
			var replace = [datetime, statut, canvasData, ej.global.imageData, ej.data.intervention.id_intervention] ;
